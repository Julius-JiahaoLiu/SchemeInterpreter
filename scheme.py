...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    >>> expr = read_line('(+ 2 2)') # Read a single string LINE as a Scheme expression
    >>> expr
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr): # Evaluate symbols by looking up their value in the current environment
        return env.lookup(expr) # It currently looks up names in the current environment
    ...
    first, rest = expr.first, expr.rest # do not mutate the passed in expr.
    
    if scheme_symbolp(first) and first in SPECIAL_FORMS: # Redirects evaluation to an appropriate do_?_form function
        return SPECIAL_FORMS[first](rest, env)
    else:
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator = scheme_eval(first, env) # evaluate the operator 
        validate_procedure(operator) # raises an error if the provided argument is not a Scheme procedure.
        if isinstance(operator, MacroProcedure):# update scheme_eval so that calls to macro procedures are evaluated correctly.
            return scheme_eval(operator.apply_macro(rest, env), env) 
        operands = rest.map(lambda x: scheme_eval(x, env)) # Evaluate all of the operands. 
        ...
        return scheme_apply(operator, operands, env) # applies a Scheme procedure to a Scheme list of arguments.
        # END PROBLEM 4
...
def scheme_apply(procedure, args, env):
    ...
    else: # procedure is a user-defined procedure
        new_env = procedure.make_call_frame(args, env) # creates a new call frame 
        return eval_all(procedure.body, new_env) # and calls eval_all on the body of the procedure, resulting in a mutually recursive eval-apply loop.
...
def eval_all(expressions, env):
    ...
    while exprs.rest is not nil:
        ...
        exprs = exprs.rest 
    return scheme_eval(exprs.first, env, True) # In a tail context should be evaluated by calling scheme_eval with True as a third argument.
    # END PROBLEM 7
...
class Frame(object):
    ...
    def __init__(self, parent):
        ...
        self.bindings = {} #It maps Scheme symbols (represented as Python strings) to Scheme values.
        self.parent = parent # The parent of the Global Frame is None.
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        if symbol in self.bindings.keys(): # self.bindings.get(symbol, None) is not None
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('{} formals can not be matched with {} vals'.formats(len(formals), len(vals)))
        else:
            child = Frame(self)
            f, v = formals, vals
            while f is not nil:
                child.define(f.first, v.first) # remember that formals and vals are Pair list!!!
                f, v = f.rest, v.rest 
            return child 
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    """A Scheme Built-in procedures are applied by calling a corresponding Python function"""

    def __init__(self, fn, use_env=False, name='builtin'):
        self.name = name
        self.fn = fn # Python function that implements the built-in Scheme procedure.
        self.use_env = use_env # a Boolean flag that indicates whether or not this built-in procedure will expect the current environment to be passed in as the last argument.
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        arg = args # do not mutate the passed in args!
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        The env provided as an argument to make_call_frame is instead the frame in which the procedure is called, 
        which will be useful as you implement dynamically scoped procedures in problem 15."""
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        new_env = self.env.make_child_frame(self.formals, args)
        return  new_env # self is just a procedure but self.env is a Frame!
        # END PROBLEM 11
...
class MacroProcedure(LambdaProcedure):
    ...
    def apply_macro(self, operands, env):
        ...
        return complete_apply(self, operands, env) # Apply procedure to operands in env and ensure the result is not a Thunk
...
def do_if_form(expressions, env):
    """Evaluate an if form. Test whether a value is a true or false value using the provided Python functions 
    is_true_primitive and is_false_primitive, defined in scheme_builtins.py.
    ...
    3
    """
    validate_form(expressions, 2, 3) # at least two but no more than three expressions.
    if is_true_primitive(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True)
        # in a tail context should be evaluated by calling scheme_eval with True as a third argument.
...
def do_and_form(expressions, env):
    ...
    while expr is not nil:
        value = scheme_eval(expr.first, env, True if expr.rest is nil else False)
        # in a tail context should be evaluated by calling scheme_eval with True as a third argument.
        if is_false_primitive(value): # Python function defined in scheme_builtins.py
            return False 
        else:
            expr = expr.rest
    return value 
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            # When true predicate does not have a corresponding result sub-expression, return the predicate value
...
def do_let_form(expressions, env):
    """Evaluate a let form. Binding symbols to values locally!!!
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    binds = bindings
    while binds is not nil:
        # if not scheme_listp(binds.first):
        #     raise SchemeError('each item must be a list containing a symbol and a Scheme expression')
        validate_form(binds.first, 2, 2)
        names = Pair(binds.first.first, names)
        values = Pair(scheme_eval(binds.first.rest.first, env), values) # ??? Why not binds.first.rest ???
        binds = binds.rest 
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    Create a MacroProcedure and bind it to the given name as in do_define_form
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        validate_formals(formals)
        body = expressions.rest 
        env.define(name, MacroProcedure(formals, body, env)) # bind the MacroProcedure to the given name 
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {}'.format(bad_target))
    # END Problem 20
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        return  new_env # Using make_child_frame() method in LambdaProcedure!!!
    # END PROBLEM 15
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 18 
    "*** YOUR CODE HERE ***"
    # Implement do_mu_form to evaluate the mu special form, evaluates to a MuProcedure instance that is dynamically scoped.
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        """Evaluate Scheme expression EXPR in environment ENV. 
        All expressions throughout your interpreter that are in a tail context should be evaluated by calling scheme_eval with True as a third argument.
        If TAIL, return a Thunk containing an expression for further evaluation.
        """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            # scheme_optimized_eval receives a non-atomic expression in a tail context, then it returns an Thunk instance.
            return Thunk(expr, env)
        result = Thunk(expr, env)
        # BEGIN
        "*** YOUR CODE HERE ***"
        # Otherwise, it should repeatedly call prior_eval_function until the result is a value, rather than a Thunk.
        ...
        try:
            ...
            if report_errors:
                if isinstance(err, SyntaxError):
                    err = SchemeError(err) # SchemeErrors are errors in user programs.
                    raise err
