...
def scheme_read(src):
    """Read and return the next single complete expression in the buffer
    Removes enough tokens from src to form a single expression in the correct Python representation
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        return Pair(quotes[val], Pair(scheme_read(src), nil)) # 'bagel should be represented as Pair('quote', Pair('bagel', nil))
        # END PROBLEM 6
    elif val not in DELIMITERS: # then it must be a primitive expression (i.e. a number, boolean)
        return val
    else:
        raise SyntaxError('unexpected token: {0}'.format(val))
def read_tail(src):
    """Read expressions (and thus remove tokens) until the matching closing parenthesis ) is seen.
    Assuming the open parenthesis of that list or pair has already been removed by scheme_read
    List of expressions is returned as a linked list of Pair instances.
    ...
    try:
        if src.current() is None:
            raise SyntaxError('unexpected end of file') # list is missing a close parenthesis
        elif src.current() == ')':
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            src.pop_first()
            ...
            second = read_tail(src) # Read the rest of the combination until the matching closing parenthesis
